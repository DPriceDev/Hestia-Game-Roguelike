    ...
    while (overlapsExist) {
        ...
        std::for_each(pRooms.begin(), pRooms.end(), [&](auto &room) {
            ...
            std::copy_if(pRooms.begin(), pRooms.end(), std::back_inserter(overlappingSame), [&](auto &otherRoom) {
             ...
             auto matchSeparate = [&room, &overlapsExist](const auto &a, const auto &b) {
                overlapsExist = true;
            ...
            const auto matchSame = [&room, &overlapsExist](const auto &a, const auto &b) {
                overlapsExist = true;
            ...
            room->mMovement += std::accumulate(std::cbegin(overlapping),
                                               std::cend(overlapping),
                                               HGE::Vector2i(),
                                               matchSeparate);
