 ...
 * @return
 */
HGE::Vector2i DungeonGenerator::randomPointInCircle() {
 ...
 * @return
 */
auto DungeonGenerator::separateRooms(std::vector<std::unique_ptr<Room>> &rooms) {
    ...
    while(overlapsExist) {
        ...
        std::for_each(pRooms.begin(), pRooms.end(), [&] (auto & room) {
            ...
            room->mMovement += std::accumulate(overlappingSame.begin(), overlappingSame.end(), HGE::Vector2i(),matchSame);
        });
 ...
 * @return
 */
HGE::Grid<std::unique_ptr<GridTile>> DungeonGenerator::createDungeonGridFromRooms(std::vector<std::unique_ptr<Room>>&rooms) {
 ...
 * @return
 */
auto DungeonGenerator::extractSmallAreaRoomsFromVector(std::vector<std::unique_ptr<Room>> &rooms) {
 ...
 */
HGE::Vector2i DungeonGenerator::pointOnRoomClosestToPoint(Room* room, HGE::Vector2i point) {
